...
def _run_translation_inference(args: argparse.Namespace) -> dict:
    ...
    return {
        "predictions": dict(predictions_by_direction),
        "counts": dict(counts),
        "timings": timings,
...
def run_translation_inference(
    ...
    *,
    ...
    language_pairs: str | list[str] = "all",
    direction: str = "forward",
    output_dir: str = "outputs/test_predictions",
    batch_size: int = 4,
    ...
    """Load a model or PEFT adapter and run translation inference."""
    if isinstance(language_pairs, (list, tuple)):
        language_pairs = ",".join(language_pairs)
    ...
    return _run_translation_inference(args)


def main() -> None:
    _run_translation_inference(parse_args())
